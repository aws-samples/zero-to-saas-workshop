...
def lambda_handler(event, context):
    ...
    #-----------------------------------------
    avpResponse = authorize_avp(event, jwt_bearer_token)
    if avpResponse['decision'] == 'DENY':
        authResponse['policyDocument']['Statement'][0]['Effect'] = 'Deny'

    #----------------------------------------
    #Lab3 - ABAC based tenant scope access policies
    #----------------------------------------
    if avpResponse['decision'] == 'ALLOW':
        credentials = get_abac_credentials(response, jwt_bearer_token, aws_account_id, identitypool_id)
        authResponse['context']['accesskey'] = credentials['AccessKeyId']
        authResponse['context']['secretkey'] = credentials['SecretKey']
        authResponse['context']['sessiontoken'] = credentials['SessionToken']
